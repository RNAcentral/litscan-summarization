...
def references_are_real(summary: str, context: str) -> bool:
    ...
    logging.warn(
        f"Model has hallucinated references! The offending items are {summary_references - context_references}"
    )
    return False
...
def not_too_many_refs_per_bracket(summary: str, context: str) -> bool:
    ...
    """
    references_in_summary = [
        len(m.split(",")) for m in re.findall(r"(\[[PMC0-9,\s]*\])", summary)
    ]
    num_context_references = len(pmcid_pattern.findall(context))

    if any(
        [
            ref_block / num_context_references > 0.5
            for ref_block in references_in_summary
        ]
    ):
        logging.warn(
            f"Model put more than half the available references in a single block!"
        )
        return False
    return True
